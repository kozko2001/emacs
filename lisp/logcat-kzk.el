...
;;;; Declarations
(defvar elogcat-pending-output "" "Stores the not processed data from logcat.")
    ...
    (with-current-buffer elogcat-buffer
      (let ((following (= (point-max) (point)))
	    ...
	    (let* ((line (substring output pos (match-beginning 0)))
		   (log-list (s-split-up-to "\s+" line 6))
...
(defun elogcat ()
  ...
  (unless (get-process "elogcat")
    (let ((proc (start-process-shell-command
		 "elogcat"
                 elogcat-buffer
		 (elogcat-command-line)
                 )))
      (set-process-filter proc 'elogcat-process-filter)
      (set-process-sentinel proc 'elogcat-process-sentinel)
      (set-process-query-on-exit-flag proc nil)   ;; make not ask permission to close the buffer
      (with-current-buffer elogcat-buffer
        (elogcat-mode t)
